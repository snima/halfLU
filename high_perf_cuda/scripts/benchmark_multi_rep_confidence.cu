...
__global__ void trtri_unit_lower_kernel(
    ...
    for (int j = 0; j < w; ++j) {
        for (int i = j + 1 + tid; i < w; i += blockDim.x) {
            float sum = 0.0f;
            for (int k = j; k < i; ++k) {
                float L_ik = (i == k) ? 1.0f : __half2float(matrix[static_cast<size_t>(offset + k) * ldl + (offset + i)]);
                sum += L_ik * s_inv[k * w + j];
            }
            s_inv[i * w + j] = -sum;
        }
        __syncthreads();
...
__global__ void store_block_kernel(
    ...
    if (r < m && c < n) {
        dst[static_cast<size_t>(col_offset + c) * ld_dst + (row_offset + r)] = src[static_cast<size_t>(c) * ld_src + r];
    }
}
...
__global__ void unified_fused_panel_kernel(
    ...
    for (int j = k; j < panel_end; ++j) {
        ...
        if (bid == 0) {
            ...
            if (method_code == 4) {
                ...
                for (int it = 0; it < 4; ++it) {
                    float my_val = 0.0f;
                    int my_c = cur_c;
                    for (int c = j + tid; c < panel_end; c += FUSED_COOP_BLOCK) {
                        float v = fabsf(__half2float(matrix[static_cast<size_t>(c) * n + cur_r]));
                        if (v > my_val) {
                            my_val = v;
                            my_c   = c;
                        }
        ...
        if (pivot_row != j) {
            ...
            // In ScPP, swap corresponding row scale factors so scaling follows rows!
        ...
        // Phase 3: Scale column j below diagonal by pivot inverse
        const __half p_elem = matrix[static_cast<size_t>(j) * n + j];
        const float pivot_val = __half2float(p_elem);
        const float inv_p = (fabsf(pivot_val) > 1e-12f) ? (1.0f / pivot_val) : 0.0f;
        const __half inv_pivot = __float2half(inv_p);

        for (int r = j + 1 + gid; r < n; r += gsize) {
            size_t idx = static_cast<size_t>(j) * n + r;
            matrix[idx] = __hmul(matrix[idx], inv_pivot);
        }
        grid.sync();

        // Phase 4: Cooperative Rank-1 Update
        const int rem_cols = panel_end - (j + 1);
        const int rem_rows = n - (j + 1);
        if (rem_cols > 0 && rem_rows > 0) {
            for (int col_offset = 0; col_offset < rem_cols; ++col_offset) {
                const int cur_col = (j + 1) + col_offset;
                const __half u_val = matrix[static_cast<size_t>(cur_col) * n + j];
                if (__half2float(u_val) == 0.0f) continue;
                for (int r = (j + 1) + gid; r < n; r += gsize) {
                    size_t l_idx = static_cast<size_t>(j) * n + r;
                    size_t a_idx = static_cast<size_t>(cur_col) * n + r;
                    __half l_val = matrix[l_idx];
                    matrix[a_idx] = __hsub(matrix[a_idx], __hmul(l_val, u_val));
                }
            }
        }
        grid.sync();
...
public:
    ...
    LookaheadTransposedFactorizer(int n, int macro_w = 512, int micro_w = 32)
        ...
        cudaOccupancyMaxActiveBlocksPerMultiprocessor(
            &max_blocks_per_sm, unified_fused_panel_kernel, FUSED_COOP_BLOCK, 0
        );
        fused_num_blocks_ = std::min(num_sm * std::max(max_blocks_per_sm, 1), 160);

        CUDA_CHECK(cudaStreamCreateWithFlags(&stream_panel_, cudaStreamNonBlocking));
        CUDA_CHECK(cudaStreamCreateWithFlags(&stream_gemm_, cudaStreamNonBlocking));
    ...
    void launch_laswp_disjoint(int k1, int k2, const int* d_ipiv, int row_start, int row_end, cudaStream_t stream) {
        int num_rows = row_end - row_start;
        if (num_rows <= 0) return;
        int num_pairs = (num_rows + 1) / 2;
        int block_dim = 256;
        int grid_dim = (num_pairs + block_dim - 1) / block_dim;
        batched_laswp_transposed_disjoint_kernel<<<grid_dim, block_dim, 0, stream>>>(
            d_B_, n_, k1, k2, d_ipiv, row_start, row_end
        );
    }
    ...
    void factorize_macro_panel(int k_macro, int W_actual, int method_code, float tau) {
        ...
        dim3 pgrid((m_panel + TILE_DIM - 1) / TILE_DIM, (W_actual + TILE_DIM - 1) / TILE_DIM);

        // Transpose slice from B into panel buffer (contiguous column-major)
        transpose_submatrix_kernel<<<pgrid, tblk, 0, stream_panel_>>>(
        ...
        for (int j_micro = 0; j_micro < W_actual; j_micro += wb) {
            ...
            float* d_row_scales_slice = d_row_scales_ + k_macro;
    ...
    void factorize(__half* d_A, int method_code, float tau) {
        ...
        launch_laswp_disjoint(0, n, d_log_pivot_rows_, 0, n, stream_panel_);
...
void run_campaign(std::vector<int> orders) {
    if (orders.empty()) {
        orders = {512, 2048, 10240, 40960, 61440};
    }
    ...
    std::cout << "================================================================================" << std::endl;
    std::cout << "  RIGOROUS MULTI-REPETITION STATISTICAL BENCHMARK FOR ALL 7 POLICIES ON V100    " << std::endl;
    std::cout << "  (Including Validated ScPP Row-Scales Precomputation and Cache Profiling)      " << std::endl;
    std::cout << "================================================================================" << std::endl;

    std::vector<std::vector<PolicyStat>> all_stats;

    for (int n : orders) {
        std::cout << "\n>>> Benchmarking Order n = " << n << " ... " << std::endl;
        size_t bytes = static_cast<size_t>(n) * n * sizeof(__half);
        std::vector<__half> h_A(static_cast<size_t>(n) * n);
        std::mt19937_64 rng(1337 + n);
        ...
        CUDA_CHECK(cudaMemcpy(d_A_orig, h_A.data(), bytes, cudaMemcpyHostToDevice));

        int W = std::min(n, 512);
        int wb = std::min(W, 32);
        LookaheadTransposedFactorizer factorizer(n, W, wb);

        int warmups = (n <= 512) ? 10 : ((n <= 2048) ? 5 : 2);
        int reps    = (n <= 512) ? 15 : ((n <= 2048) ? 10 : ((n <= 10240) ? 5 : 5));
        ...
        for (const auto& m_pair : methods) {
            ...
            // Compute statistical mean and standard error
            float sum = 0.0f;
            ...
            float sem = std_dev / std::sqrt(static_cast<float>(times.size()));

            double total_flops = (2.0 / 3.0) * std::pow(static_cast<double>(n), 3.0);
            double tflops = (total_flops / (mean * 1e-3)) * 1e-12;
            ...
            float overhead = (code == 0) ? 0.0f : ((mean - pp_mean) / pp_mean * 100.0f);
            if (overhead < 0.0f) overhead = 0.0f;

            // Error propagation for ratio (T - T_pp) / T_pp
            float ov_sem = 0.0f;
            if (code != 0 && pp_mean > 0.0f) {
                float rel_err = std::sqrt((sem * sem + pp_sem * pp_sem) / (pp_mean * pp_mean));
                ov_sem = rel_err * 100.0f;
            }

            n_stats.push_back({n, code, name, mean, std_dev, sem, tflops, overhead, ov_sem});

            std::cout << "    [" << std::setw(4) << name << "]  "
                      << "Time = " << std::fixed << std::setprecision(2) << std::setw(8) << mean << " ms "
                      << "± " << std::setw(5) << std::setprecision(2) << sem << " ms | "
                      << std::setw(6) << std::setprecision(3) << tflops << " TFLOPS | Overhead: "
                      << std::setw(6) << std::setprecision(2) << ((code == 0) ? "Baseline" : ("+" + std::to_string(overhead).substr(0, 5) + "%"))
                      << std::endl;
        }

        all_stats.push_back(n_stats);
    ...
    std::cout << "\n\n================================================================================" << std::endl;
    std::cout << "        FINAL VALIDATED PAPER TABLE 5 (WITH CONFIDENCE INTERVALS ON V100)       " << std::endl;
    std::cout << "================================================================================" << std::endl;
    ...
    for (const auto& n_res : all_stats) {
        int n = n_res[0].n;
        std::string size_str = (n == 512) ? "512" : (std::to_string(n / 1024) + "K");
        float pp_ms = n_res[0].mean_ms;
        ...
        std::string pp_str = std::to_string(static_cast<int>(pp_ms)) + "." + 
                             std::to_string(static_cast<int>(pp_ms * 100) % 100);

        std::cout << std::left << std::setw(8) << size_str
                  << std::right << std::setw(10) << std::fixed << std::setprecision(2) << pp_ms
                  << " ±" << std::setw(4) << std::setprecision(2) << pp_sem
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        orders.push_back(std::atoi(argv[i]));
    }
    run_campaign(orders);
    return 0;
}
