...
void test_fused_blocks(int m, int W, int wb, int num_blocks) {
    __half *d_panel;
    ...
    for (int j = 0; j < W; j += wb) {
        ...
        if (rest > 0) {
            ...
            CUBLAS_CHECK(cublasHgemm(
                ...
                &h_one,
                d_L_inv, wb_act,
                d_panel + (j + static_cast<size_t>(j + wb_act) * m), m,
                &h_zero,
                d_u12, wb_act
            ));
            dim3 sblk(32, 8);
            ...
            CUBLAS_CHECK(cublasHgemm(
                ...
                m_trail, rest, wb_act,
                &h_minus_one,
                d_panel + ((j + wb_act) + static_cast<size_t>(j) * m), m,
                d_u12, wb_act,
                &h_one,
                d_panel + ((j + wb_act) + static_cast<size_t>(j + wb_act) * m), m
            ));
        }
    }
    ...
    std::cout << "m = " << m << ", W = " << W << ", num_blocks = " << num_blocks 
...
int main() {
    ...
    int blocks_arr[] = {40, 80, 160, 320, 640};
    for (int b : blocks_arr) {
        test_fused_blocks(m, W, wb, b);
    }
    return 0;
}
