...
void check_cublas(cublasStatus_t status, const char* expression, const char* file, int line) {
    if (status != CUBLAS_STATUS_SUCCESS) {
        std::ostringstream message;
        message << file << ':' << line << ": cuBLAS call " << expression << " failed with status " << status;
        throw std::runtime_error(message.str());
    }
}

#define CUBLAS_CHECK(expression) check_cublas((expression), #expression, __FILE__, __LINE__)
...
struct Options {
    ...
    int norm_probes = 0;
    bool sampled_reconstruction = true;
    bool track_growth = false;
    int ir_iterations = 0;
    double ir_tolerance = 1.0e-12;
...
struct RunSummary {
    ...
    double exact_frobenius_residual = std::numeric_limits<double>::quiet_NaN();
    int norm_probe_count = 0;
    double frobenius_residual_estimate = std::numeric_limits<double>::quiet_NaN();
    double frobenius_residual_rse = std::numeric_limits<double>::quiet_NaN();
    double frobenius_residual_ci_lower = std::numeric_limits<double>::quiet_NaN();
    double frobenius_residual_ci_upper = std::numeric_limits<double>::quiet_NaN();
    float norm_probe_ms = 0.0f;
    double initial_max_abs = std::numeric_limits<double>::quiet_NaN();
    double certified_growth_envelope = std::numeric_limits<double>::quiet_NaN();
    int ir_iterations_completed = 0;
    bool ir_converged = false;
    double ir_initial_backward_error = std::numeric_limits<double>::quiet_NaN();
    double ir_final_backward_error = std::numeric_limits<double>::quiet_NaN();
    double ir_final_forward_error = std::numeric_limits<double>::quiet_NaN();
    float ir_ms = 0.0f;
};
...
struct ReconstructionResult {
    ...
    double relative_frobenius = std::numeric_limits<double>::quiet_NaN();
};

struct NormProbeResult {
    int count = 0;
    double relative_frobenius = std::numeric_limits<double>::quiet_NaN();
    double relative_standard_error = std::numeric_limits<double>::quiet_NaN();
...
struct IterativeRefinementResult {
    ...
    double initial_backward_error = std::numeric_limits<double>::quiet_NaN();
    double final_backward_error = std::numeric_limits<double>::quiet_NaN();
    double final_forward_error = std::numeric_limits<double>::quiet_NaN();
    float elapsed_ms = 0.0f;
};
...
__global__ void generate_norm_probe_kernel(
    double* probe,
    double* scaled_probe,
    const int* exponents,
    int n,
    ...
    for (int column = blockIdx.x * blockDim.x + threadIdx.x; column < n;
        ...
        const std::uint64_t key = seed ^
            (static_cast<std::uint64_t>(probe_index + 1) * 0x9e3779b97f4a7c15ULL) ^
            static_cast<std::uint64_t>(column);
        const double sign = (mix64(key) & 1ULL) ? 1.0 : -1.0;
        probe[column] = sign;
        scaled_probe[column] = ldexp(sign, exponents[column]);
    }
}

__global__ void half_matrix_matvec_fp64_kernel(
    const __half* matrix, const double* vector, double* output, int n) {
    for (int row = blockIdx.x * blockDim.x + threadIdx.x; row < n;
         row += blockDim.x * gridDim.x) {
        double accumulator = 0.0;
        const unsigned long long offset = static_cast<unsigned long long>(row) * n;
        for (int column = 0; column < n; ++column) {
            accumulator += static_cast<double>(__half2float(matrix[offset + column])) * vector[column];
        }
        output[row] = accumulator;
    }
}

__global__ void packed_upper_matvec_fp64_kernel(
    const __half* factors, const double* scaled_probe, double* output, int n) {
    for (int row = blockIdx.x * blockDim.x + threadIdx.x; row < n;
         row += blockDim.x * gridDim.x) {
        double accumulator = 0.0;
        const unsigned long long offset = static_cast<unsigned long long>(row) * n;
        for (int column = row; column < n; ++column) {
            accumulator += static_cast<double>(__half2float(factors[offset + column])) * scaled_probe[column];
        }
        output[row] = accumulator;
    }
}

__global__ void packed_lower_matvec_fp64_kernel(
    const __half* factors, const double* upper_product, double* output, int n) {
    for (int row = blockIdx.x * blockDim.x + threadIdx.x; row < n;
         row += blockDim.x * gridDim.x) {
        double accumulator = upper_product[row];
        const unsigned long long offset = static_cast<unsigned long long>(row) * n;
        for (int column = 0; column < row; ++column) {
            accumulator += static_cast<double>(__half2float(factors[offset + column])) * upper_product[column];
        }
        output[row] = accumulator;
    }
}

__global__ void permuted_residual_norm_kernel(
    const double* input_product,
    const double* factor_product,
    ...
    for (int row = blockIdx.x * blockDim.x + threadIdx.x; row < n;
         row += blockDim.x * gridDim.x) {
        const double residual = input_product[permutation[row]] - factor_product[row];
    ...
    if (threadIdx.x == 0) atomicAdd(norm_squared, sums[0]);
}

__global__ void half_matrix_frobenius_kernel(
    const __half* matrix, unsigned long long count, double* norm_squared) {
    __shared__ double sums[kThreads];
    double local = 0.0;
    for (unsigned long long index = static_cast<unsigned long long>(blockIdx.x) * blockDim.x + threadIdx.x;
         index < count; index += static_cast<unsigned long long>(blockDim.x) * gridDim.x) {
        const double value = static_cast<double>(__half2float(matrix[index]));
        local += value * value;
    }
    sums[threadIdx.x] = local;
    __syncthreads();
...
__global__ void convert_half_to_float_kernel(
    const __half* input, float* output, unsigned long long count) {
    for (unsigned long long index = static_cast<unsigned long long>(blockIdx.x) * blockDim.x + threadIdx.x;
         index < count; index += static_cast<unsigned long long>(blockDim.x) * gridDim.x) {
...
__global__ void fill_double_kernel(double* vector, int n, double value) {
    for (int index = blockIdx.x * blockDim.x + threadIdx.x; index < n;
         index += blockDim.x * gridDim.x) vector[index] = value;
}

__global__ void half_matrix_row_abs_sum_kernel(const __half* matrix, double* sums, int n) {
    for (int row = blockIdx.x * blockDim.x + threadIdx.x; row < n;
         row += blockDim.x * gridDim.x) {
        double sum = 0.0;
        const unsigned long long offset = static_cast<unsigned long long>(row) * n;
        for (int column = 0; column < n; ++column) {
            sum += fabs(static_cast<double>(__half2float(matrix[offset + column])));
        }
        sums[row] = sum;
    }
}

__global__ void permute_double_to_float_kernel(
    const double* input, const int* permutation, float* output, int n) {
    for (int row = blockIdx.x * blockDim.x + threadIdx.x; row < n;
         row += blockDim.x * gridDim.x) {
        output[row] = static_cast<float>(input[permutation[row]]);
    }
}

__global__ void ledger_unscale_float_to_double_kernel(
    const float* input, const int* exponents, double* output, int n) {
    for (int column = blockIdx.x * blockDim.x + threadIdx.x; column < n;
         column += blockDim.x * gridDim.x) {
        output[column] = ldexp(static_cast<double>(input[column]), -exponents[column]);
    }
}

__global__ void residual_double_kernel(
    const double* right_hand_side, const double* product, double* residual, int n) {
    for (int row = blockIdx.x * blockDim.x + threadIdx.x; row < n;
         row += blockDim.x * gridDim.x) residual[row] = right_hand_side[row] - product[row];
}

__global__ void add_double_vectors_kernel(double* solution, const double* correction, int n) {
    for (int index = blockIdx.x * blockDim.x + threadIdx.x; index < n;
         index += blockDim.x * gridDim.x) solution[index] += correction[index];
}
...
__global__ void predictive_guard_columns_kernel(
    ...
    unsigned long long* growth_max_bits) {
    ...
    if (threadIdx.y == 0) {
        ...
        if (column < col_end) {
            ...
            if (growth_max_bits != nullptr && isfinite(rounded_bound)) {
                const double original_coordinate_bound = ldexp(
                    static_cast<double>(rounded_bound), cumulative_exponents[column]);
                if (isfinite(original_coordinate_bound) && original_coordinate_bound >= 0.0) {
                    atomicMax(growth_max_bits, static_cast<unsigned long long>(__double_as_longlong(original_coordinate_bound)));
                }
            }
...
__global__ void update_growth_from_bound_kernel(
    const float* bound,
    const int* cumulative_exponents,
    int col_start,
    int col_end,
    unsigned long long* growth_max_bits) {
    for (int column = col_start + blockIdx.x * blockDim.x + threadIdx.x; column < col_end;
         column += blockDim.x * gridDim.x) {
        const double original_coordinate_bound = ldexp(
            static_cast<double>(bound[column]), cumulative_exponents[column]);
        if (isfinite(original_coordinate_bound) && original_coordinate_bound >= 0.0) {
            atomicMax(growth_max_bits, static_cast<unsigned long long>(__double_as_longlong(original_coordinate_bound)));
        }
    }
...
void launch_predictive_guard_device(
    ...
    int col_end,
    float threshold,
    unsigned long long* growth_max_bits = nullptr) {
...
NormProbeResult estimate_frobenius_residual(
    const DeviceBuffer<__half>& initial,
    const DeviceBuffer<__half>& factors,
    const DeviceBuffer<int>& exponents,
    const std::vector<int>& permutation,
    int n,
    int probe_count,
    std::uint64_t seed,
    GpuTimer& timer) {
    NormProbeResult result;
    if (probe_count <= 0 || n <= 0) return result;

    DeviceBuffer<double> d_probe(n);
    DeviceBuffer<double> d_scaled_probe(n);
    DeviceBuffer<double> d_input_product(n);
    DeviceBuffer<double> d_upper_product(n);
    DeviceBuffer<double> d_factor_product(n);
    DeviceBuffer<double> d_sum(1);
    DeviceBuffer<int> d_permutation(n);
    CUDA_CHECK(cudaMemcpy(
        d_permutation.get(), permutation.data(), permutation.size() * sizeof(int), cudaMemcpyHostToDevice));

    std::vector<double> probe_norms;
    probe_norms.reserve(probe_count);
    double input_norm_squared = 0.0;
    const unsigned long long element_count = static_cast<unsigned long long>(n) * n;
    const int vector_blocks = blocks_for(static_cast<unsigned long long>(n));
    result.elapsed_ms = timer.measure([&] {
        CUDA_CHECK(cudaMemset(d_sum.get(), 0, sizeof(double)));
        half_matrix_frobenius_kernel<<<blocks_for(element_count), kThreads>>>(
            initial.get(), element_count, d_sum.get());
        CUDA_CHECK(cudaGetLastError());
        CUDA_CHECK(cudaMemcpy(&input_norm_squared, d_sum.get(), sizeof(double), cudaMemcpyDeviceToHost));

        for (int probe_index = 0; probe_index < probe_count; ++probe_index) {
            generate_norm_probe_kernel<<<vector_blocks, kThreads>>>(
                d_probe.get(), d_scaled_probe.get(), exponents.get(), n, seed, probe_index);
            half_matrix_matvec_fp64_kernel<<<vector_blocks, kThreads>>>(
                initial.get(), d_probe.get(), d_input_product.get(), n);
            packed_upper_matvec_fp64_kernel<<<vector_blocks, kThreads>>>(
                factors.get(), d_scaled_probe.get(), d_upper_product.get(), n);
            packed_lower_matvec_fp64_kernel<<<vector_blocks, kThreads>>>(
                factors.get(), d_upper_product.get(), d_factor_product.get(), n);
            CUDA_CHECK(cudaMemset(d_sum.get(), 0, sizeof(double)));
            permuted_residual_norm_kernel<<<vector_blocks, kThreads>>>(
                d_input_product.get(), d_factor_product.get(), d_permutation.get(), n, d_sum.get());
            CUDA_CHECK(cudaGetLastError());
            double norm_squared = 0.0;
            CUDA_CHECK(cudaMemcpy(&norm_squared, d_sum.get(), sizeof(double), cudaMemcpyDeviceToHost));
            probe_norms.push_back(norm_squared);
        }
    });

    const double mean = std::accumulate(probe_norms.begin(), probe_norms.end(), 0.0) / probe_norms.size();
    double variance = 0.0;
    if (probe_norms.size() > 1) {
        for (const double value : probe_norms) variance += (value - mean) * (value - mean);
        variance /= static_cast<double>(probe_norms.size() - 1);
    }
    const double standard_error = std::sqrt(variance / probe_norms.size());
    const double denominator = std::max(input_norm_squared, std::numeric_limits<double>::min());
    result.count = probe_count;
    result.relative_frobenius = std::sqrt(std::max(0.0, mean) / denominator);
    result.relative_standard_error = mean > 0.0 ? 0.5 * standard_error / mean : 0.0;
    result.ci_lower = std::sqrt(std::max(0.0, mean - 1.96 * standard_error) / denominator);
    result.ci_upper = std::sqrt(std::max(0.0, mean + 1.96 * standard_error) / denominator);
    return result;
}

IterativeRefinementResult run_iterative_refinement(
    const DeviceBuffer<__half>& initial,
    const DeviceBuffer<__half>& factors,
    const DeviceBuffer<int>& exponents,
    const std::vector<int>& permutation,
    ...
    double tolerance,
    GpuTimer& timer) {
    IterativeRefinementResult result;
    if (maximum_iterations <= 0 || n <= 0) return result;

    const unsigned long long element_count = static_cast<unsigned long long>(n) * n;
    const int vector_blocks = blocks_for(static_cast<unsigned long long>(n));
    DeviceBuffer<float> d_float_factors(static_cast<std::size_t>(element_count));
    DeviceBuffer<float> d_inner_rhs(n);
    DeviceBuffer<double> d_ones(n);
    DeviceBuffer<double> d_rhs(n);
    DeviceBuffer<double> d_product(n);
    DeviceBuffer<double> d_residual(n);
    DeviceBuffer<double> d_solution(n);
    DeviceBuffer<double> d_correction(n);
    DeviceBuffer<double> d_row_sums(n);
    DeviceBuffer<int> d_permutation(n);
    CUDA_CHECK(cudaMemcpy(
        d_permutation.get(), permutation.data(), permutation.size() * sizeof(int), cudaMemcpyHostToDevice));

    std::vector<double> host_rhs(n);
    std::vector<double> host_solution(n);
    std::vector<double> host_residual(n);
    std::vector<double> host_row_sums(n);
    cublasHandle_t handle{};
    CUBLAS_CHECK(cublasCreate(&handle));

    auto solve = [&](const DeviceBuffer<double>& right_hand_side, DeviceBuffer<double>& solution) {
        permute_double_to_float_kernel<<<vector_blocks, kThreads>>>(
            right_hand_side.get(), d_permutation.get(), d_inner_rhs.get(), n);
        CUDA_CHECK(cudaGetLastError());
        CUBLAS_CHECK(cublasStrsv(
            handle, CUBLAS_FILL_MODE_UPPER, CUBLAS_OP_T, CUBLAS_DIAG_UNIT,
            n, d_float_factors.get(), n, d_inner_rhs.get(), 1));
        CUBLAS_CHECK(cublasStrsv(
            handle, CUBLAS_FILL_MODE_LOWER, CUBLAS_OP_T, CUBLAS_DIAG_NON_UNIT,
            n, d_float_factors.get(), n, d_inner_rhs.get(), 1));
        ledger_unscale_float_to_double_kernel<<<vector_blocks, kThreads>>>(
            d_inner_rhs.get(), exponents.get(), solution.get(), n);
        CUDA_CHECK(cudaGetLastError());
    };

    double matrix_infinity_norm = 0.0;
    double right_hand_side_infinity_norm = 0.0;
    auto measure_errors = [&]() -> std::pair<double, double> {
        half_matrix_matvec_fp64_kernel<<<vector_blocks, kThreads>>>(
            initial.get(), d_solution.get(), d_product.get(), n);
        residual_double_kernel<<<vector_blocks, kThreads>>>(
            d_rhs.get(), d_product.get(), d_residual.get(), n);
        CUDA_CHECK(cudaGetLastError());
        CUDA_CHECK(cudaMemcpy(
            host_solution.data(), d_solution.get(), n * sizeof(double), cudaMemcpyDeviceToHost));
        CUDA_CHECK(cudaMemcpy(
            host_residual.data(), d_residual.get(), n * sizeof(double), cudaMemcpyDeviceToHost));
        double solution_norm = 0.0;
        double residual_norm = 0.0;
        double forward_error = 0.0;
        for (int index = 0; index < n; ++index) {
            solution_norm = std::max(solution_norm, std::abs(host_solution[index]));
            residual_norm = std::max(residual_norm, std::abs(host_residual[index]));
            forward_error = std::max(forward_error, std::abs(host_solution[index] - 1.0));
        }
        const double denominator = std::max(
            matrix_infinity_norm * solution_norm, std::numeric_limits<double>::min());
        return {residual_norm / denominator, forward_error};
    };

    result.elapsed_ms = timer.measure([&] {
        convert_half_to_float_kernel<<<blocks_for(element_count), kThreads>>>(
            factors.get(), d_float_factors.get(), element_count);
        fill_double_kernel<<<vector_blocks, kThreads>>>(d_ones.get(), n, 1.0);
        half_matrix_matvec_fp64_kernel<<<vector_blocks, kThreads>>>(
            initial.get(), d_ones.get(), d_rhs.get(), n);
        half_matrix_row_abs_sum_kernel<<<vector_blocks, kThreads>>>(
            initial.get(), d_row_sums.get(), n);
        CUDA_CHECK(cudaGetLastError());
        CUDA_CHECK(cudaMemcpy(host_rhs.data(), d_rhs.get(), n * sizeof(double), cudaMemcpyDeviceToHost));
        CUDA_CHECK(cudaMemcpy(
            host_row_sums.data(), d_row_sums.get(), n * sizeof(double), cudaMemcpyDeviceToHost));
        for (int index = 0; index < n; ++index) {
            right_hand_side_infinity_norm = std::max(right_hand_side_infinity_norm, std::abs(host_rhs[index]));
            matrix_infinity_norm = std::max(matrix_infinity_norm, host_row_sums[index]);
        }

        solve(d_rhs, d_solution);
        auto [backward_error, forward_error] = measure_errors();
        result.initial_backward_error = backward_error;
        result.final_backward_error = backward_error;
        result.final_forward_error = forward_error;
        result.converged = std::isfinite(backward_error) && backward_error <= tolerance;

        for (int iteration = 1; iteration <= maximum_iterations && !result.converged; ++iteration) {
            solve(d_residual, d_correction);
            add_double_vectors_kernel<<<vector_blocks, kThreads>>>(
                d_solution.get(), d_correction.get(), n);
            CUDA_CHECK(cudaGetLastError());
            std::tie(backward_error, forward_error) = measure_errors();
            result.iterations_completed = iteration;
            result.final_backward_error = backward_error;
            result.final_forward_error = forward_error;
            result.converged = std::isfinite(backward_error) && backward_error <= tolerance;
            if (!std::isfinite(backward_error) || !std::isfinite(forward_error)) break;
        }
    });
...
ReconstructionResult reconstruct_lu(
    ...
    for (int row_index = 0; row_index < sample_dimension; ++row_index) {
        ...
        for (int column_index = 0; column_index < sample_dimension; ++column_index) {
            ...
            if (n <= 256) {
                input_sum_squared += permuted_input * permuted_input;
                const double difference = reconstructed - permuted_input;
                residual_sum_squared += difference * difference;
            }
        }
    }
    result.relative_residual = max_residual / std::max(max_input, std::numeric_limits<double>::min());
    if (n <= 256) {
        result.relative_frobenius = std::sqrt(residual_sum_squared /
            std::max(input_sum_squared, std::numeric_limits<double>::min()));
    }
    return result;
}
...
RunSummary run_lu_sample(const Options& options, int sample, std::vector<PanelRecord>* records) {
    std::vector<float> host_float = make_lu_input(options, options.seed + static_cast<std::uint64_t>(sample));
    ...
    DeviceBuffer<unsigned long long> d_growth_max_bits(1);
    GpuTimer timer;
    ...
    CUDA_CHECK(cudaMemset(d_growth_max_bits.get(), 0, sizeof(unsigned long long)));
    ...
    if (options.scaling_policy == ScalingPolicy::kCertified ||
        ...
        if (options.track_growth) {
            summary.initial_max_abs = static_cast<double>(
                read_bound_max(d_bound, 0, options.n, d_scratch_uint));
        }
    }
    if (options.scaling_policy == ScalingPolicy::kPredictive) {
        ...
        preflight_ms += guard_timer.measure([&] {
            launch_predictive_guard_device(
                ...
                options.track_growth ? d_growth_max_bits.get() : nullptr);
        });
            ...
            if (options.blocking != Blocking::kUnblocked && panel_end < options.n) {
                ...
                if (options.scaling_policy == ScalingPolicy::kPredictive) {
                    reanchor_column_bound_kernel<<<blocks_for(static_cast<unsigned long long>(width)), kThreads>>>(
                        d_bound.get(), d_fused_max_bits.get(), panel_end, options.n);
                    if (options.track_growth) {
                        update_growth_from_bound_kernel<<<blocks_for(static_cast<unsigned long long>(width)), kThreads>>>(
                            d_bound.get(), d_exponents.get(), panel_end, options.n, d_growth_max_bits.get());
                    }
                    CUDA_CHECK(cudaGetLastError());
                }
            }
    ...
    if (options.track_growth && std::isfinite(summary.initial_max_abs) && summary.initial_max_abs > 0.0) {
        unsigned long long growth_bits = 0;
        CUDA_CHECK(cudaMemcpy(
            &growth_bits, d_growth_max_bits.get(), sizeof(growth_bits), cudaMemcpyDeviceToHost));
        double growth_maximum = 0.0;
        std::memcpy(&growth_maximum, &growth_bits, sizeof(growth_maximum));
        summary.certified_growth_envelope = growth_maximum / summary.initial_max_abs;
    }
    if (keep_running && summary.final_nonfinite == 0) {
        ...
        std::vector<int> permutation(options.n);
        std::iota(permutation.begin(), permutation.end(), 0);
        summary.row_swaps = 0;
        for (int pivot = 0; pivot < options.n; ++pivot) {
            if (pivots[pivot] < 0 || pivots[pivot] >= options.n) {
                summary.reconstruction_kind = "invalid_pivots";
                return summary;
            }
        ...
        if (options.ir_iterations > 0) {
            const IterativeRefinementResult refinement = run_iterative_refinement(
                d_initial,
                d_matrix,
                d_exponents,
                permutation,
                options.n,
                options.ir_iterations,
                options.ir_tolerance,
                timer);
            summary.ir_iterations_completed = refinement.iterations_completed;
            summary.ir_converged = refinement.converged;
            summary.ir_initial_backward_error = refinement.initial_backward_error;
            summary.ir_final_backward_error = refinement.final_backward_error;
            summary.ir_final_forward_error = refinement.final_forward_error;
            summary.ir_ms = refinement.elapsed_ms;
        }

        if (options.norm_probes > 0) {
            const NormProbeResult norm_probe = estimate_frobenius_residual(
                d_initial,
                d_matrix,
                d_exponents,
                permutation,
                options.n,
                options.norm_probes,
                options.seed + static_cast<std::uint64_t>(sample),
                timer);
            summary.norm_probe_count = norm_probe.count;
            summary.frobenius_residual_estimate = norm_probe.relative_frobenius;
            summary.frobenius_residual_rse = norm_probe.relative_standard_error;
            summary.frobenius_residual_ci_lower = norm_probe.ci_lower;
            summary.frobenius_residual_ci_upper = norm_probe.ci_upper;
            summary.norm_probe_ms = norm_probe.elapsed_ms;
        }

        if (options.sampled_reconstruction) {
            ...
            summary.exact_frobenius_residual = reconstruction.relative_frobenius;
        } else {
            summary.reconstruction_kind = options.norm_probes > 0
                ? "sampled_disabled_norm_probe_primary" : "reconstruction_disabled";
        }
...
void write_metadata(const Options& options, const std::filesystem::path& output) {
    ...
    file << "  \"norm_probes\": " << options.norm_probes << ",\n";
    file << "  \"sampled_reconstruction\": " << (options.sampled_reconstruction ? "true" : "false") << ",\n";
    file << "  \"track_growth\": " << (options.track_growth ? "true" : "false") << ",\n";
    file << "  \"ir_iterations\": " << options.ir_iterations << ",\n";
    file << "  \"ir_tolerance\": " << options.ir_tolerance << ",\n";
    ...
    file << "    \"reconstruction_check\": \"GPU FP64 randomized norm probing; optional host sampled check\",\n";
...
void write_run_summaries(const std::filesystem::path& output, const std::vector<RunSummary>& summaries) {
    ...
    file << "sample,completed,termination,panels_observed,proxy_activations,oracle_activations,true_positive,"
            ...
            "exact_frobenius_residual,"
            "norm_probe_count,frobenius_residual_estimate,frobenius_residual_rse,"
            "frobenius_residual_ci_lower,frobenius_residual_ci_upper,norm_probe_ms,"
            "initial_max_abs,certified_growth_envelope,ir_iterations_completed,ir_converged,"
            "ir_initial_backward_error,ir_final_backward_error,ir_final_forward_error,ir_ms\n";
    for (const RunSummary& summary : summaries) {
        file << summary.sample << ',' << (summary.completed ? 1 : 0) << ',' << summary.termination << ','
             ...
             << summary.reconstruction_relative_residual << ',' << summary.exact_frobenius_residual << ','
             << summary.norm_probe_count << ','
             << summary.frobenius_residual_estimate << ',' << summary.frobenius_residual_rse << ','
             << summary.frobenius_residual_ci_lower << ',' << summary.frobenius_residual_ci_upper << ','
             << summary.norm_probe_ms << ',' << summary.initial_max_abs << ','
             << summary.certified_growth_envelope << ',' << summary.ir_iterations_completed << ','
             << (summary.ir_converged ? 1 : 0) << ',' << summary.ir_initial_backward_error << ','
             << summary.ir_final_backward_error << ',' << summary.ir_final_forward_error << ','
             << summary.ir_ms << '\n';
    }
}
...
void print_summary(const std::vector<RunSummary>& summaries, const std::filesystem::path& output) {
    ...
    for (const RunSummary& summary : summaries) {
        std::cout << "sample=" << summary.sample << " completed=" << (summary.completed ? "yes" : "no")
                  ...
                  << " fro_residual=" << summary.frobenius_residual_estimate
                  << " probe_rse=" << summary.frobenius_residual_rse
                  << " ir=" << summary.ir_iterations_completed
                  << " ir_backward=" << summary.ir_final_backward_error << '\n';
    }
}
...
void print_usage(const char* program) {
    std::cout << "Usage: " << program << " [options]\n\n"
              ...
              << "  --norm-probes INTEGER      randomized Frobenius probes; default: 0\n"
              << "  --sampled-reconstruction on|off default: on\n"
              << "  --track-growth on|off      certified original-coordinate envelope; default: off\n"
              << "  --ir-iterations INTEGER    ledger-aware FP32 inner solve; default: 0\n"
              << "  --ir-tolerance FLOAT       matrix-only backward-error target; default: 1e-12\n"
...
Options parse_options(int argc, char** argv) {
    ...
    for (int index = 1; index < argc; ++index) {
        ...
        } else if (argument == "--norm-probes") {
            options.norm_probes = parse_integer(option_value(argc, argv, &index, "--norm-probes"), "--norm-probes");
        } else if (argument == "--sampled-reconstruction") {
            const std::string value = option_value(argc, argv, &index, "--sampled-reconstruction");
            if (value == "on") {
                options.sampled_reconstruction = true;
            } else if (value == "off") {
                options.sampled_reconstruction = false;
            } else {
                throw std::runtime_error("--sampled-reconstruction must be on or off");
            }
        } else if (argument == "--track-growth") {
            const std::string value = option_value(argc, argv, &index, "--track-growth");
            if (value == "on") {
                options.track_growth = true;
            } else if (value == "off") {
                options.track_growth = false;
            } else {
                throw std::runtime_error("--track-growth must be on or off");
            }
        } else if (argument == "--ir-iterations") {
            options.ir_iterations = parse_integer(
                option_value(argc, argv, &index, "--ir-iterations"), "--ir-iterations");
        } else if (argument == "--ir-tolerance") {
            options.ir_tolerance = parse_float(
                option_value(argc, argv, &index, "--ir-tolerance"), "--ir-tolerance");
    ...
    if (options.norm_probes < 0) {
        throw std::runtime_error("--norm-probes must be non-negative");
    }
    if (options.norm_probes > 0 && options.mode != Mode::kLu) {
        throw std::runtime_error("--norm-probes is supported only in lu mode");
    }
    if (options.ir_iterations < 0 || options.ir_tolerance <= 0.0) {
        throw std::runtime_error("IR requires non-negative iterations and a positive tolerance");
    }
    if (options.ir_iterations > 0 && options.mode != Mode::kLu) {
        throw std::runtime_error("iterative refinement is supported only in lu mode");
    }
    ...
    if (options.scaling_policy == ScalingPolicy::kPredictive) {
        if (options.mode != Mode::kLu ||
            (options.blocking != Blocking::kBlockedFp16 &&
             options.blocking != Blocking::kBlockedFp32Accumulate)) {
            throw std::runtime_error("predictive scaling requires lu mode with blocked_fp16 or blocked_fp32");
        }
        if (!options.scaling) {
